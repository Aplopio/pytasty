...
def dehydrate(detail_object):
    ...
    for field_name in [name for name in dir(detail_object) if not name.startswith('_') and not name.startswith("json") and name not in ["save", "delete", "get_file"]]:
        field_schema = detail_object._list_object.schema[
            'fields'].get(field_name)
        if not field_schema:
            detail_object.__dict__[field_name] = getattr(
                detail_object,
                field_name)
            continue

        field_handler = get_field_handler(field_schema)
        dehydrated_value = field_handler.dehydrate(
            getattr(
                detail_object,
                field_name),
            parent_obj=detail_object)
...
def get_schema(
        ...
        schema_endpoint,
        from_dump=True,
        schema_dump_uri=None):
    if from_dump:
        if not hasattr(api_client.utils, "_schema_dump") and schema_dump_uri is not None:
